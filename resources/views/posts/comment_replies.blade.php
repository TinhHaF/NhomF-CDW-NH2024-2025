<div class="flex items-start space-x-4 {{ $depth < 3 ? 'ml-' . ($depth * 4) : '' }}">
    <div>
        ...
        <div class="flex space-x-4 p-4 border-b border-gray-300 bg-gray-50">
            ...
            <div class="flex-shrink-0">
                <img alt="User avatar" class="w-12 h-12 rounded-full border-2 border-blue-500 object-cover" height="40"
                    width="40" src="{{ $comment->user->image
    ...
    : asset('user_avt/avt.jpg') }}">
        ...
        <form action="{{ route('comments.user_delete', $comment->encoded_comment_id) }}" method="POST"
            class="flex items-center space-x-4 mt-2">
            @csrf
            @method('DELETE')
            <button onclick="toggleReplyForm('{{ $comment->comment_id }}', '{{ $comment->user->username }}')"
                type="button" class="text-blue-500 hover:underline">
                Trả lời
            </button>
            <button onclick="toggleReplies('{{ $comment->comment_id }}')" type="button"
        ...
        <!-- Form trả lời ẩn -->
        <form id="reply-form-{{ $comment->comment_id }}" action="{{ route('comments_store', ['post' => $post->id]) }}"
            method="POST" class="hidden mt-4">
            @csrf
            <input type="hidden" name="parent_id" value="{{ $comment->comment_id }}">
            <textarea id="reply-textarea-{{ $comment->comment_id }}" class="w-full p-2 border rounded-lg mb-2"
                name="content" placeholder="Nhập trả lời của bạn" rows="2" oninput="autoResize(this)"
                required></textarea>
